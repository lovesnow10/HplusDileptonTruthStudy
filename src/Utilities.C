...
{
    std::cout<<"WARNING: You are creating new file, will overwrite exist file with the same name"<<std::endl;
...
TTree* GetTTree(const char* fTreeName, TFile* fFile)
{
    ...
    if (fKey != nullptr) {
        ...
        std::cout<<"WARNING: Cannot find TTree with name "<<fTreeName<<", please check. ";
        std::cout<<"Return nullptr..."<<std::endl;
    }
    return fTree;
}
