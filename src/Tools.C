...
{
  std::vector<int> pdgid = GetTreeValue<std::vector<int>>(mEvent, "truth_pdgid");
  std::vector<float> truth_pt = GetTreeValue<std::vector<float>>(mEvent, "truth_pt");
  std::vector<float> truth_eta = GetTreeValue<std::vector<float>>(mEvent, "truth_eta");
  std::vector<float> truth_phi = GetTreeValue<std::vector<float>>(mEvent, "truth_phi");
  std::vector<float> truth_m = GetTreeValue<std::vector<float>>(mEvent, "truth_m");
  ...
  for (size_t i = 0; i < nParticles; i++) {
    ...
    if (id == -11 || id == -13)
    {
      LpVect->SetPtEtaPhiM(truth_pt.at(i), truth_eta.at(i), truth_phi.at(i), truth_m.at(i));
      returnID = id;
      break;
    }
  }
...
int GetLeptonMinusTruth(TTree *mEvent, TLorentzVector *LmVect)
{
  ...
  for (size_t i = 0; i < nParticles; i++) {
    ...
    if (id == 11 || id == 13)
    {
      LmVect->SetPtEtaPhiM(truth_pt.at(i), truth_eta.at(i), truth_phi.at(i), truth_m.at(i));
      returnID = id;
      break;
    }
  }
...
int GetLeptonPlusDetector(TTree *mEvent, TLorentzVector *LpVect)
{
  ...
  std::vector<float> el_eta = GetTreeValue<std::vector<float>>(mEvent, "el_eta");
  std::vector<float> el_phi = GetTreeValue<std::vector<float>>(mEvent, "el_phi");
  ...
  std::vector<float> mu_eta = GetTreeValue<std::vector<float>>(mEvent, "mu_eta");
  std::vector<float> mu_phi = GetTreeValue<std::vector<float>>(mEvent, "mu_phi");
  std::vector<float> mu_e = GetTreeValue<std::vector<float>>(mEvent, "mu_e");
  std::vector<float> el_charge = GetTreeValue<std::vector<float>>(mEvent, "el_charge");
  std::vector<float> mu_charge = GetTreeValue<std::vector<float>>(mEvent, "mu_charge");

  int nElectrons = GetTreeValue<int>(mEvent, "nElectrons");
  int nMuons = GetTreeValue<int>(mEvent, "nMuons");

  int returnID = 0; //this indicate leading or sub-leading lepton, only for ee and mumu
  ...
  if (nDilType == 2)
  {
    if (el_charge.at(0) == 1) returnID = 1;
    else returnID = 2;
    LpVect->SetPtEtaPhiE(el_pt.at(returnID-1), el_eta.at(returnID-1), el_phi.at(returnID-1), el_e.at(returnID-1));
  }
  else if (nDilType == -2)
  {
    if (mu_charge.at(0) == 1) returnID = 1;
    else returnID = 2;
    LpVect->SetPtEtaPhiE(mu_pt.at(returnID-1), mu_eta.at(returnID-1), mu_phi.at(returnID-1), mu_e.at(returnID-1));
  }
  else if (nDilType == 0)
  {
    if (el_charge.at(0) == 1) returnID = 1;
    else returnID = 2;
    if (returnID == 1) LpVect->SetPtEtaPhiE(el_pt.at(0), el_eta.at(0), el_phi.at(0), el_e.at(0));
    else LpVect->SetPtEtaPhiE(mu_pt.at(0), mu_eta.at(0), mu_phi.at(0), mu_e.at(0));
  }
  return returnID;
}
...
int GetJetDetector(TTree *mEvent, TLorentzVector *BxVect, ObjType _type)
{
  ...
  std::vector<float> jet_pt = GetTreeValue<std::vector<float>>(mEvent, "jet_pt");
  std::vector<float> jet_eta = GetTreeValue<std::vector<float>>(mEvent, "jet_eta");
  std::vector<float> jet_phi = GetTreeValue<std::vector<float>>(mEvent, "jet_phi");
  ...
  for (size_t i = 0; i < nJets; i++) {
    ...
    {
      returnID = Jet_bit;
      BxVect->SetPtEtaPhiE(jet_pt.at(i), jet_eta.at(i), jet_phi.at(i), jet_e.at(i));
      break;
    }
  }
  return returnID;
...
std::map<std::string, bool> LepMatching(TTree* mEvent)
{
  ...
  if (dR_Lp < DR_CUT) results["Lp"] = true;
  if (dR_Lm < DR_CUT) results["Lm"] = true;
...
int FakeLeptonRemoval(TTree *mEvent)
{
  ...
  std::vector<int> el_true_type = GetTreeValue<std::vector<int>>(mEvent, "el_true_type");
  std::vector<int> mu_true_type = GetTreeValue<std::vector<int>>(mEvent, "mu_true_type");
  std::vector<int> el_true_origin = GetTreeValue<std::vector<int>>(mEvent, "el_true_origin");
  std::vector<int> mu_true_origin = GetTreeValue<std::vector<int>>(mEvent, "mu_true_origin");
  std::vector<int> el_true_originbkg = GetTreeValue<std::vector<int>>(mEvent, "el_true_originbkg");
  ...
  if (nDilType == 2)
  {
    ...
    pass = (lt1 == 2 && (lo1 == 10 || lo1 == 12 || lo1 == 13 || lo1 == 14)) || (lt1 == 4 && lo1 == 5 && (lb1 == 10 || lb1 == 12 || lb1 == 13 || lb1 == 14));
    pass = pass && ((lt2 == 2 && (lo2 == 10 || lo2 == 12 || lo2 == 13 || lo2 == 14)) || (lt2 == 4 && lt2 == 5 && (lb2 == 10 || lb2 == 12 || lb2 == 13 || lb2 == 14)));
  }
  else if (nDilType == -2)
  {
    ...
    pass = lt1 == 6 && lt2 == 6 && (lo1 == 10 || lo1 == 12 || lo1 == 13 || lo1 == 14) && (lo2 == 10 || lo2 == 12 || lo2 == 13 || lo2 == 14);
  }
  else if (nDilType == 0)
  {
    lt1 = el_true_type.at(0);
    ...
    pass = pass && ((lt1 == 2 && (lo1 == 10 || lo1 == 12 || lo1 == 13 || lo1 == 14)) || (lt1 == 4 && lo1 == 5 && (lb1 == 10 || lb1 == 12 || lb1 == 13 || lb1 == 14)));
  }
...
int CheckJetsWiLepMatchingEff(TTree* mTree, const char* outName)
{
    ...
    for (long i = 0; i < nentries; ++i) {
        ...
        if (b1_match && b2_match && b3_match && lp_match && lm_match) {
            ...
            if (b0_match) h_4j3bIncRawEvents->Fill(7);
        }
        if (nJets > 3 && nBTags == 3)
        {
            mTotal4j3bRawEvents++;
            ...
            if (b2_match) h_4j3bRawEvents->Fill(2);
            if (b3_match) h_4j3bRawEvents->Fill(3);
            if (lp_match) h_4j3bRawEvents->Fill(4);
            if (lm_match) h_4j3bRawEvents->Fill(5);
            if (b1_match && b2_match && b3_match && lp_match && lm_match) {
                mTotalUsefulEvent4j3b++;
                h_4j3bRawEvents->Fill(6);
                if (b0_match) h_4j3bRawEvents->Fill(7);
            }
        } else if (nJets > 3 && nBTags >3) {
            mTotal4j4bRawEvents++;
            if (b0_match) h_4j4bRawEvents->Fill(0);
            ...
            if (b1_match && b2_match && b3_match && lm_match && lp_match) {
                ...
                if (b0_match) h_4j4bRawEvents->Fill(7);
            }
        }
    }
    std::cout<<"Loop ended, saving information to Histos..."<<std::endl;
